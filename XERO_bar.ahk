...
APPVER := 23                            ; 앱 버전 — 수정할 때마다 +1 (제목에 v20 처럼 표시)
...
DESC := Map(
    ...
    "increaseapply",    ["인보이스의 NOTICE 줄을 읽어 가격 인상을 자동 적용합니다 (%, 새 가격, /w /b 범위 지원).", "Reads the NOTICE line and auto-applies the price increase (%, new prices, /w /b scopes)."],
    "stripeoff",        ["온라인 결제(Stripe)를 끄고 저장합니다.", "Turns off Stripe online payment and saves."],
    "stripeoffsave",    ["Stripe를 끄고, 이메일 발송 없이 저장만 합니다.", "Turns off Stripe, then saves with NO email sent."],
    "app4sending",      ["Approve for Sending 선택 + 저장 (PDF 첨부 · Mark as sent · Done 자동).", "Selects Approve for Sending + saves (auto PDF attach, Mark as sent, Done)."],
    "stripesending",    ["Stripe Off + App4Sending 을 한 번에 실행합니다.", "Stripe Off + App4Sending in one click."],
...
helpFor := Map()                        ; 버튼 hwnd → 도구 id (우클릭 설명용)
...
Build() {
    ...
    g.OnEvent("ContextMenu", CtxHelp)   ; 버튼 우클릭 = 설명
